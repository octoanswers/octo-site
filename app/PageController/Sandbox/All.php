...
class All_Sandbox_PageController extends Abstract_PageController
{
    ...
    {
        return $this->translator->get('sandbox', 'title').' - '._('Page').' '.$this->page.' - '.$this->translator->get('answeropedia');
    }

    public function _getPageDescription(): string
    {
        $description = $this->translator->get('sandbox', 'title').' - '._('Page').' '.$this->page.' - '.$this->translator->get('answeropedia');
